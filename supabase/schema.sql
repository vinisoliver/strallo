...
--
--   * `parent_uuid` e `collection_uuid` não têm FOREIGN KEY de propósito. A
--     ordem de chegada das linhas não é garantida: uma subcoleção pode subir
--     antes da pasta que a contém, e a chave estrangeira recusaria a inserção
--     de algo que está correto e só chegou primeiro.
...
-- Sem RLS as três tabelas seriam legíveis por qualquer pessoa com a chave
...
declare
  t text;
begin
  foreach t in array array['collections', 'cards', 'practice_sessions'] loop
